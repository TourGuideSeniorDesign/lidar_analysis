...
public:
  LidarDetectionNode()
  : Node("lidar_detection_with_distance")
  {
    ...
    // Create a PCL Visualizer (runs in the same process)
...
private:
  ...
  void updateViewer()
  {
    ...
    // 3) Color each cluster differently, draw bounding boxes, add distance text
    int i = 0;
    ...
    // Auto-reset camera so we’re not stuck zoomed in:
    // viewer_->resetCamera();
